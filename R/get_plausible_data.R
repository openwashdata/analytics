...
#'
#' This function fetches analytics data from the Plausible API for a given website and saves it into CSV files.
#' The data includes metrics like pageviews, visitors, bounce rate, and more. The function allows customization
#' of the website URL and the option to retrieve pagewise data.
#'
#' @param site_url A character string specifying the website URL to retrieve data for. If left empty,
#' the default URL "ds4owd-001.github.io/website" is used.
#' @param pagewise A logical value indicating whether to retrieve pagewise breakdown data. Default is `FALSE`.
#' @param token A character string for the Plausible API token. Defaults to retrieving the token from
#' the environment variable "PLAUSIBLE_TOKEN". See the Readme for more information on how to save tokens
#' to the R environment.
#'
#' @details
#' The function constructs API requests to Plausible to retrieve data for countries, sources, browsers,
#' and a time series of visits. It saves the data in CSV files within a folder named after the site.
#'
#' The data is broken down by country, source, and browser. Optionally, if `pagewise = TRUE`, it also retrieves
#' pagewise data.
#'
...
get_plausible_data <- function(site_url = "", pagewise = FALSE, token = Sys.getenv("PLAUSIBLE_TOKEN")) {
  ...
  # If site url is not specified, use default
  if (site_url == "") {
    site_url <- SITE_URL
  ...
  if (pagewise) {
    ...
    pageviews_data <- get_data(url_pageviews, token)
  }
  ...
  readr::write_csv(country_data, file.path("data", folder_name, paste(folder_name, "country_data.csv", sep = "_")))
  readr::write_csv(source_data, file.path("data", folder_name, paste(folder_name, "source_data.csv", sep = "_")))
  readr::write_csv(browser_data, file.path("data", folder_name, paste(folder_name, "browser_data.csv", sep = "_")))
  readr::write_csv(timeseries_data, file.path("data", folder_name, paste(folder_name, "timeseries_data.csv", sep = "_")))

  print(paste("Successfully saved data to", folder_name))
}

#' Send Request to Plausible for Data
#'
#' @param url A character string specifying the URL to send the request to.
#' @param TOKEN A character string specifying the authentication token.
#'
...
get_data <- function(url, TOKEN) {
  ...
  if (httr::status_code(response) == 200) {
    ...
    results <- jsonlite::fromJSON(data)$results

    # Return the results
    return(results)
  } else {
    print("Could not retrieve data, please check your authentication token or URL")
    return(NULL)
  }
}
